...
exports.create = function(conf){
    ...
    if(conf.flag === 'project'){
        ...
        exports.aimee.log('Start  create', color.purple(conf.flag), color.green(conf.name), '...');
    }

    // Folder
    conf.type === 'folder' && this.folder(conf.path);
    // Html
    conf.type === 'html' && this.html(conf.path);
    // Text File
    conf.type === 'text' && this.file(conf.path);

    // Aimee Page
    conf.flag === 'page' && this.app(conf.path, 'page');
    // Aimee app
    conf.flag === 'app' && this.app(conf.path, 'app');
