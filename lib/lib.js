...
lib.extend({
    ...
    // 创建 ~/.aimeerc
    createAimeeRc: function(rc){
        rc = path.join(process.env.HOME, '.aimeerc');
        lib.isFile(rc) || fs.createWriteStream(rc);
    },

    // 创建 ~/.aimee
    createAimeeFolder: function(folder){
        folder = config.get('dir.cache');
        lib.isDir(folder) || lib.mkdir(folder);
    },
    ...
    getRC: function(){
        ...
        aimeerc = rc.get();
        aimeerc.core = aimeerc.core || {};
        aimeerc.user = aimeerc.user || {};
